...
export function SiteApp({ 
  ...
  // View Overrides for Platform Admins (Will hide role selector so user gets a pristine physical view)
  ...
  const [materialsLibraryForPrices, setMaterialsLibraryForPrices] = useState<any[]>([
    ...
    { item: 'Coarse Aggregate 20mm', unit: 'm³', standardCost: 32.00, currentCost: 32.00, group: 'Civil' },
  ...
  const handleAlertSubmit = async () => {
    ...
    try {
      ...
      // Try inserting with 'message' first
      ...
      if (error) {
        // Fall back to description if message column is missing
        if (error.message?.includes('column "message" does not exist') || error.message?.includes('column') || error.message?.includes('message')) {
          console.warn('Fallback: column "message" missing, trying with "description" column instead.');
                                ...
                                <div className="flex items-start justify-between relative">
                                  <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                      <span className="px-1.5 py-0.5 bg-surface-base border border-border-subtle rounded text-[7px] font-mono font-black text-primary uppercase">
  ...
  const renderAlerts = () => (
    <div className="flex flex-col h-full bg-surface-base">
          ...
          <div className="flex flex-col gap-4 mt-6">
            ...
            {notifications.length === 0 ? (
              ...
              <div key={i} className="p-6 bg-surface-1 border border-border-subtle rounded-[2rem] border-l-8 border-l-danger shadow-sm group hover:shadow-md transition-all">
                ...
                <p className="text-[11px] text-ghost mt-2 leading-relaxed font-medium">{a.message || a.description}</p>
      ...
      <AnimatePresence>
        {isReportingAlert && (
          <div className={cn(
            "z-[2000] bg-black/60 backdrop-blur-md flex items-center justify-center p-4 overflow-hidden",
        ...
        <div className="flex flex-col gap-4">
          <div className="p-4 bg-surface-1 border border-border-subtle rounded-2xl flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-ghost">Verify offsite and locally-purhcased goods receipts prior to stock check-in</p>
            </div>
          </div>
