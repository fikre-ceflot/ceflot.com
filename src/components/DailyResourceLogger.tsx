...
export function DailyResourceLogger({ dailyProgressId, tenantId, onClose, date }: DailyResourceLoggerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-surface-base/95 backdrop-blur-md z-[100] flex flex-col animate-in fade-in duration-300">
      {/* Banners */}
      {draftRestored && (
        <div className="bg-primary/90 backdrop-blur text-white px-4 py-2 text-center text-xs font-bold animate-in slide-in-from-top duration-300">
           Draft restored for this report.
        </div>
      )}
      {saveState === 'saved' && (
        <div className="bg-emerald-500 text-white px-4 py-2 text-center text-xs font-bold animate-in slide-in-from-top duration-300">
           Daily resources saved successfully.
        </div>
      )}
      {saveState === 'error' && (
        <div className="bg-danger text-white px-4 py-3 flex items-center justify-between gap-4 animate-in slide-in-from-top duration-300">
      ...
      {/* Header */}
      <div className="h-16 border-b border-border-subtle flex items-center justify-between px-6 bg-surface-1">
        <div className="flex items-center gap-4">
          <button onClick={onClose} className="p-2 hover:bg-surface-2 rounded-lg transition-all text-ghost hover:text-main">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <div>
            <h2 className="text-sm font-bold text-main">Daily Resource Allocation</h2>
            <p className="text-[10px] font-mono text-primary uppercase tracking-widest">{new Date(date).toLocaleDateString(undefined, { dateStyle: 'full' })}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex flex-col items-end mr-4">
            <span className="text-[10px] font-bold text-ghost uppercase tracking-widest">Est. Daily Cost</span>
            <span className="text-lg font-mono font-bold text-primary">${calculateTotalCost().toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
          </div>
          <button 
            onClick={handleSave}
            disabled={saving || saveState === 'saving'}
            className="flex items-center gap-2 px-4 py-2 bg-primary hover:brightness-110 text-surface-base font-bold rounded-lg transition-all disabled:opacity-50 min-w-[140px] justify-center"
          >
            {saveState === 'saving' ? (
              <>
          ...
          <button onClick={onClose} className="p-2 hover:bg-surface-2 rounded-lg transition-all text-ghost">
            <X className="w-5 h-5" />
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar Tabs */}
        <div className="w-64 border-r border-border-subtle bg-surface-1 p-4 flex flex-col gap-2">
          <button 
            onClick={() => setActiveTab('labour')}
            className={cn(
              "flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all",
              activeTab === 'labour' ? "bg-surface-2 text-primary shadow-lg shadow-black/20" : "text-ghost hover:bg-surface-2 hover:text-main"
            )}
          >
          ...
          <button 
            ...
            className={cn(
              "flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all",
              activeTab === 'equipment' ? "bg-surface-2 text-primary shadow-lg shadow-black/20" : "text-ghost hover:bg-surface-2 hover:text-main"
            )}
          >
          ...
          <button 
            ...
            className={cn(
              "flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all",
              activeTab === 'materials' ? "bg-surface-2 text-primary shadow-lg shadow-black/20" : "text-ghost hover:bg-surface-2 hover:text-main"
            )}
          >
          ...
          <button 
            ...
            className={cn(
              "flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all",
              activeTab === 'progress' ? "bg-surface-2 text-primary shadow-lg shadow-black/20" : "text-ghost hover:bg-surface-2 hover:text-main"
            )}
          >
        ...
        {/* Content Area */}
        <div className="flex-1 overflow-y-auto p-8 bg-surface-base">
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-lg font-bold text-main capitalize">{activeTab} Details</h3>
                <p className="text-xs text-ghost">Log detailed {activeTab} resources for this daily report.</p>
              </div>
              <button 
                onClick={() => addItem(activeTab)}
                className="flex items-center gap-2 px-3 py-1.5 bg-surface-2 border border-border-subtle text-primary text-xs font-bold rounded-lg hover:bg-surface-3 transition-all"
              >
            ...
            {loading ? (
              <div className="py-20 text-center text-ghost animate-pulse">Loading daily resources...</div>
            ) : (
              <div className="space-y-3">
                {activeTab === 'labour' && dailyLabour.map((item) => (
                  <div key={item.id} className="bg-surface-1 border border-border-subtle rounded-xl p-4 flex items-center gap-4 animate-in slide-in-from-left-2 duration-300">
                    <div className="flex-1">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Labour Grade</label>
                      <select 
                        value={item.labour_grade_id}
                        onChange={e => updateItem('labour', item.id, 'labour_grade_id', e.target.value)}
                        className="w-full bg-surface-base border border-border-subtle rounded-lg px-3 py-2 text-sm text-main focus:border-primary outline-none"
                      >
                    ...
                    <div className="w-32">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Count</label>
                      <input 
                        type="number"
                        value={item.count}
                    ...
                    <div className="w-32">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Hours</label>
                      <input 
                        type="number"
                        step="0.5"
                        value={item.hours}
                    ...
                    <button 
                      ...
                      className="mt-5 p-2 text-ghost hover:text-danger transition-colors"
                    >
                ...
                {activeTab === 'equipment' && dailyEquipment.map((item) => (
                  <div key={item.id} className="bg-surface-1 border border-border-subtle rounded-xl p-4 flex items-center gap-4 animate-in slide-in-from-left-2 duration-300">
                    <div className="flex-1">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Equipment Item</label>
                      <select 
                    ...
                    <div className="w-32">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Hours Used</label>
                      <input 
                        type="number"
                        step="0.5"
                        value={item.hours}
                    ...
                    <div className="w-32">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Fuel (L)</label>
                      <input 
                        type="number"
                        step="1"
                ...
                {activeTab === 'materials' && dailyMaterials.map((item) => (
                  <div key={item.id} className="bg-surface-1 border border-border-subtle rounded-xl p-4 flex items-center gap-4 animate-in slide-in-from-left-2 duration-300">
                    <div className="flex-1">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Material</label>
                      <select 
                    ...
                    <div className="w-48">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Quantity Used</label>
                      <input 
                        type="number"
                        step="0.01"
                        value={item.quantity}
                ...
                {activeTab === 'progress' && dailyActivities.map((item) => (
                  <div key={item.id} className="bg-surface-1 border border-border-subtle rounded-xl p-4 flex items-center gap-4 animate-in slide-in-from-left-2 duration-300">
                    <div className="flex-[2]">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">BOQ Activity</label>
                      <select 
                    ...
                    <div className="flex-1">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Subcontractor (Optional)</label>
                      <select 
                    ...
                    <div className="w-32">
                      <label className="text-[10px] font-bold text-ghost uppercase mb-1 block">Qty Done</label>
                      <input 
                        type="number"
                        step="0.01"
                ...
                {((activeTab === 'labour' && dailyLabour.length === 0) || 
                  ...
                  <div className="py-20 border-2 border-dashed border-border-subtle rounded-2xl text-center">
                    <div className="text-ghost text-sm">No {activeTab} logged for this report.</div>
                    <button 
                      onClick={() => addItem(activeTab)}
