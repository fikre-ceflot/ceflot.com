...
import { TRADE_GROUPS, getGroupLabel, getGroupEmoji, OPERATIONAL_BUNDLES } from '../lib/constants';
...
export function TradeLibrary({ userRole, tenantId, isGodMode }: TradeLibraryProps) {
  ...
  const [selectedItem, setSelectedItem] = useState<GroupedTradeItem | null>(null);
  ...
  const [showCalculator, setShowCalculator] = useState<{ index: number; row: any } | null>(null);
  ...
  const fetchAllResources = async () => {
    ...
    try {
      const [mats, labs, eqps, vehs, fuels] = await Promise.all([
        supabase.from('materials').select('id, material_name, material_code, unit').eq('is_active', true),
        supabase.from('labour_grades').select('id, title, grade_code').eq('is_active', true) as any,
        supabase.from('equipment_items').select('id, name, equipment_code').eq('is_active', true),
        supabase.from('vehicles').select('id, name, vehicle_code').eq('is_active', true),
        supabase.from('fuel_types').select('id, name, fuel_code, unit').eq('is_active', true),
      ]);
  ...
  const enrichResources = async (resources: TradeItemRow[]) => {
    setIsSyncing(true);
    try {
      const enriched = await Promise.all(resources.map(async (res) => {
        let table = '';
        switch (res.resource_type?.toLowerCase()) {
          case 'material': table = 'materials'; break;
          case 'labour': table = 'labour_grades'; break;
          case 'equipment': table = 'equipment_items'; break;
          case 'vehicle': table = 'vehicles'; break;
          case 'fuel': table = 'fuel_types'; break;
          default: return { ...res, not_found: false };
        }
  ...
  const loadTradeLibrary = async () => {
    setLoading(true);
    try {
      console.log(`[TradeLibrary] Loading trade_items. TenantID: ${tenantId}, GodMode: ${isGodMode}`);
  ...
  const handleSave = async () => {
    ...
    if (!formData.trade_code || !formData.trade_item || !formData.trade_group) {
      setNotification({ type: 'error', message: 'Please fill in all required fields (Code, Group, Item Name).' });
      return;
    }

    setLoading(true);
    try {
      const tier = editingItem ? editingItem.tier : (isGodMode && tierFilter === 'global' ? 'global' : 'company');
      ...
      setNotification({ type: 'success', message: editingItem ? 'Trade item updated successfully!' : 'Trade item saved successfully!' });
      ...
      alert('Error saving trade item: ' + e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      ...
      if (recipeDraft.length > 0) {
        ...
        const { error: insError } = await supabase.from('trade_items').insert(finalRows);
        ...
        const { error: insError } = await supabase.from('trade_items').insert([placeholder]);
        if (insError) throw insError;
      }
  ...
  const cloneGlobalTrades = async (specificCodes?: string[]) => {
    if (!tenantId) {
      setNotification({ type: 'error', message: 'Cannot clone: Tenant ID is missing. Please refresh or log in again.' });
      return;
    }
    ...
    if (tradeCodesToClone.size === 0) {
      setNotification({ type: 'info', message: 'No global trade items selected to clone.' });
      return;
    }
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const existingKeys = new Set(
            existingTrades?.map(t => `${(t.trade_code || '').toLowerCase().trim()}|${(t.resource_code || '').toLowerCase().trim()}`) || []
          );

          const existingCodes = new Set(
            existingTrades?.map(t => (t.trade_code || '').toLowerCase().trim()) || []
          );

          // 2. Fetch global database entries for non-synthetic items
          const dbCodesToQuery = Array.from(tradeCodesToClone).filter(c => !c.startsWith('OP-'));
          let dbRows: any[] = [];
          ...
          const clones = filteredResources.map(g => {
            const { id, created_at, library_tier, tenant_id, effective_rate, ...rest } = g as any;
            return {
              ...rest,
              version: g.version || 1,
          ...
          if (finalClonesToInsert.length > 0) {
            const { error } = await supabase.from('trade_items').insert(finalClonesToInsert);
            if (error) throw error;
          }
          ...
          setNotification({ 
            type: (tradeCodesToClone.size - skippedCount) > 0 ? 'success' : 'info', 
            message: msg 
          });
          ...
          setNotification({ type: 'error', message: 'Error cloning trades: ' + e.message });
        } finally {
          setLoading(false);
    ...
    setConfirmAction({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          setNotification({ 
            type: 'success', 
            message: isCompany ? 'Trade item deleted permanently.' : 'Trade item deactivated successfully.' 
          });
          loadTradeLibrary();
        } catch (err: any) {
          setNotification({ type: 'error', message: 'Error deleting trade: ' + err.message });
        } finally {
          setLoading(false);
          setConfirmAction(null);
        }
  ...
  const handleExcelImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const formattedData = data.map((row: any) => {
        const code = row.activity_code || row.Code || row.code || row['Trade Code'] || row.trade_code || row.ID || row.id;
        const group = row.work_group || row.Group || row.group || row['Trade Group'] || row.trade_group || row.Category || row.category;
        const item = row.activity_name || row.Item || row.item || row['Trade Item'] || row.trade_item || row.Description || row.description || row.Name || row.name;
        const unit = row.boq_unit || row.Unit || row.unit || row['BOQ Unit'] || row.boq_unit || 'm3';
        const resType = row.resource_type || row.ResourceType || row.resource_type || row.Type || row.type || 'material';
        const resCode = row.resource_code || row.ResourceCode || row.resource_code || row.Code || row.code;
        const resName = row.resource_name || row.ResourceName || row.resource_name || row.Resource || row.resource || row.Material || row.material || row.Labour || row.labour || row.Description || row.description || row.Name || row.name;
        const resUnit = row.resource_unit || row.ResourceUnit || row.resource_unit || row['Res Unit'] || row.res_unit || row.Unit || row.unit || 'Unit';
        
        // Distinguish between Consumption (Qty)
        const consumption = parseFloat(row.consumption_rate || row.Consumption || row.consumption_rate || row.Qty || row.qty || row.Amount || 1);

        if (!tenantId && !isGodMode) {
          throw new Error('Tenant ID missing during import. Please ensure you are logged in.');
        }
        ...
        return {
          ...
          waste_factor_pct: parseFloat(row.waste_factor_pct || row.Waste || row.waste_factor_pct || row['Waste %'] || 0),
          labour_hours_per_unit: parseFloat(row.labor_hours_per_unit || row.LabHrs || row.labour_hours_per_unit || row.Hours || row.hours || 0),
          daily_output_qty: parseFloat(row.daily_output_qty || row.Output || row.daily_output_qty || row.Daily || row.daily || 0),
          team_setup: row.team_setup || row.Team || row.team_setup || row.Crew || row.crew,
          tenant_id: tenantId,
      ...
      if (formattedData.length === 0) {
        alert('No valid data found in Excel. Ensure you have "Code" and "Item" columns.');
        ...
        return;
          ...
          const { data: existingEntries } = await supabase
            ...
            .eq('tenant_id', tenantId)
            .eq('library_tier', isGodMode && tierFilter === 'global' ? 'global' : 'company');
          
          const existingMap = new Map(existingEntries?.map(e => [`${e.trade_code}|${e.resource_name}`.toLowerCase(), e.id]) || []);
          ...
          const previewRows = formattedData.map(item => {
            ...
            return {
              ...
              status: existingId ? (importMode === 'overwrite' ? 'update' : 'skip') : 'new'
            };
          });
  ...
  const confirmBatchImport = async () => {
    ...
    try {
      ...
      if (toInsert.length === 0 && toUpdate.length === 0) {
        alert('No new or updated items to import.');
  ...
  const downloadTemplate = async () => {
    ...
    const data = [
      { 
        activity_code: 'MOB-1', work_group: 'Mobilization', activity_name: 'General Mobilization & Site Setup', 
        boq_unit: 'LS', resource_type: 'Material', resource_code: 'MAT-FU-001', resource_name: 'Diesel AGO', resource_unit: 'Litre',
        consumption_rate: 5000, waste_factor_pct: 0, labor_hours_per_unit: 8, daily_output_qty: 1, team_setup: 'Site Team + Logistics' 
      },
      { 
        activity_code: 'EX-1', work_group: 'Excavation', activity_name: 'Excavation Site Clearance', 
        boq_unit: 'M2', resource_type: 'Labour', resource_code: 'LAB-US-001', resource_name: 'General Labourer', resource_unit: 'Day',
        consumption_rate: 0.025, waste_factor_pct: 0, labor_hours_per_unit: 8, daily_output_qty: 400, team_setup: '1 Foreman + 8 Labourers' 
      }
    ];
  ...
  const filteredGroups = React.useMemo(() => {
    return groups.map(g => ({
      ...
      items: g.items.filter(i => {
        ...
        if (tierFilter === 'all' && i.tier === 'global' && companyTradeCodes.has(i.code.toLowerCase().trim())) {
          return false;
        }
  ...
  const getResourceIcon = (type: string) => {
    switch (type.toLowerCase()) {
      ...
      case 'labour': return <Users className="w-3.5 h-3.5" />;
      case 'equipment': return <Wrench className="w-3.5 h-3.5" />;
      case 'vehicle': return <Truck className="w-3.5 h-3.5" />;
      case 'fuel': return <Droplet className="w-3.5 h-3.5" />;
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-dim">Manage construction trade items and their resource recipes</p>
        </div>
              ...
              {selectedCodes.size > 0 ? (
                ...
                <button 
                  onClick={() => {
                    ...
                    alert('Select items from the Global library to clone them to your company.');
                  }}
            ...
            <div className="flex bg-surface-base p-0.5 rounded-lg border border-border-subtle">
              <button
                ...
                className={cn(
                  ...
                  importMode === 'append' ? "bg-primary text-surface-base" : "text-ghost hover:text-main"
                )}
              >
                APPEND
              </button>
              <button
                ...
                className={cn(
                  "px-3 py-1 text-[9px] font-bold rounded-md transition-all",
                  importMode === 'overwrite' ? "bg-danger text-white" : "text-ghost hover:text-main"
                )}
              >
                OVERWRITE
              </button>
            ...
            <button 
              onClick={() => {
                if (!hasCapability('trade:import')) {
                  alert('You do not have permission to import trades.');
                  return;
                }
          ...
          <div className="bg-surface-2 border border-border-subtle rounded-xl max-w-5xl w-full shadow-2xl overflow-hidden flex flex-col my-8 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border-subtle bg-surface-1">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-main">
                  {editingItem ? `Edit Trade Item & Recipe: ${editingItem.code}` : 'Add New Trade Item & Recipe'}
                </h3>
              </div>
              <button 
                onClick={() => {
            ...
            <div className="p-6 overflow-y-auto max-h-[70vh] flex flex-col gap-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ghost uppercase tracking-wider">Group</label>
                    <select 
                      ...
                      onChange={e => setFormData({...formData, trade_group: e.target.value})}
                    >
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ghost uppercase tracking-wider">Code</label>
                    <input 
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ghost uppercase tracking-wider">Item Name</label>
                    <input 
                      className="bg-surface-base border border-border-subtle rounded px-3 py-2 text-sm outline-none focus:border-primary text-main"
                      value={formData.trade_item}
                      onChange={e => setFormData({...formData, trade_item: e.target.value})}
                      placeholder="e.g. Concrete C25"
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ghost uppercase tracking-wider">BOQ Unit</label>
                    <input 
                      className="bg-surface-base border border-border-subtle rounded px-3 py-2 text-sm outline-none focus:border-primary text-main"
                      value={formData.boq_unit}
                      onChange={e => setFormData({...formData, boq_unit: e.target.value})}
                      placeholder="m3"
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ghost uppercase tracking-wider">Daily Output</label>
                    <input 
                      type="number"
                      className="bg-surface-base border border-border-subtle rounded px-3 py-2 text-sm outline-none focus:border-primary text-main"
                      value={formData.daily_output_qty}
                      onChange={e => setFormData({...formData, daily_output_qty: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ghost uppercase tracking-wider">Labour Hours</label>
                    <input 
                      type="number"
                      className="bg-surface-base border border-border-subtle rounded px-3 py-2 text-sm outline-none focus:border-primary text-main"
                      value={formData.labour_hours_per_unit}
                      onChange={e => setFormData({...formData, labour_hours_per_unit: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                </div>
              </div>
              ...
              <div>
                <h4 className="text-xs font-bold text-ghost uppercase tracking-wider mb-3 border-b border-border-subtle pb-1 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono text-ghost/70 font-normal">
                    {formData.resources.length} active row{formData.resources.length !== 1 ? 's' : ''}
                  </span>
                </h4>
                
                <div className="flex flex-col gap-3">
                  {formData.resources.map((res, idx) => (
                    <div key={idx} className="bg-surface-base border border-border-subtle rounded-lg p-3 grid grid-cols-1 md:grid-cols-6 gap-3 group relative shadow-sm hover:border-border-faint transition-all">
                      <button 
                        onClick={() => {
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[9px] uppercase font-bold text-ghost">Type</label>
                        <select 
                      ...
                      <div className="flex flex-col gap-1 md:col-span-2">
                        <label className="text-[9px] uppercase font-bold text-ghost">Resource Name</label>
                        <input 
                          className="bg-surface-2 border border-border-subtle text-xs px-2 py-1.5 rounded outline-none text-main"
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[9px] uppercase font-bold text-ghost">Consumption</label>
                        <input 
                          type="number"
                          step="any"
                          className="bg-surface-2 border border-border-subtle text-xs px-2 py-1.5 rounded outline-none text-main"
                          ...
                          onChange={(e) => {
                            const next = [...formData.resources];
                            next[idx].consumption_rate = parseFloat(e.target.value) || 0;
                            setFormData({ ...formData, resources: next });
                          }}
                        />
                      </div>
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[9px] uppercase font-bold text-ghost">Unit</label>
                        <input 
                          className="bg-surface-2 border border-border-subtle text-xs px-2 py-1.5 rounded outline-none text-main"
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[9px] uppercase font-bold text-ghost">Waste %</label>
                        <input 
                          type="number"
                          className="bg-surface-2 border border-border-subtle text-xs px-2 py-1.5 rounded outline-none text-main"
                          ...
                          onChange={(e) => {
                            const next = [...formData.resources];
                            next[idx].waste_factor_pct = parseFloat(e.target.value) || 0;
                            setFormData({ ...formData, resources: next });
                          }}
                        />
                      </div>
                  ...
                  {formData.resources.length === 0 && (
                    <div className="text-center py-6 border border-dashed border-border-subtle rounded-xl text-xs text-ghost italic bg-surface-base/50">
                      No recipe components added yet. Add resources from the library or create ad-hoc rows.
                    </div>
                  )}
                    ...
                    <button 
                      onClick={() => {
                        setFormData({
                          ...
                          resources: [
                            ...formData.resources,
                            { resource_type: 'material', resource_name: '', resource_unit: '', consumption_rate: 0, waste_factor_pct: 0 }
                          ]
                        });
                      }}
                      className="flex items-center justify-center gap-2 p-2.5 border border-border-subtle border-dashed rounded-xl text-xs text-ghost hover:text-main hover:border-ghost transition-all bg-surface-base cursor-pointer"
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ghost uppercase tracking-wider">Team Setup</label>
                    <textarea 
                      ...
                      onChange={e => setFormData({...formData, team_setup: e.target.value})}
                      placeholder="Describe the crew structure, grades needed, or typical setups..."
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ghost uppercase tracking-wider">Notes & Remarks</label>
                    <textarea 
                      className="bg-surface-base border border-border-subtle rounded px-3 py-2 text-sm outline-none focus:border-primary text-main min-h-[90px]"
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border-subtle bg-surface-1">
              ...
              <button 
                ...
                disabled={loading}
              >
                {loading ? 'Saving...' : (editingItem ? 'Update Everything' : 'Save Everything')}
              </button>
            </div>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col">
              {filteredGroups.map((group) => (
                <div key={group.code} className="border-b border-border-subtle last:border-0">
                  <button 
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="text-[13px] font-bold text-main">{cleanRichText(group.name)}</div>
                      <div className="text-[10px] font-mono text-ghost uppercase tracking-wider">{cleanRichText(group.code)} • {group.items.length} Items</div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-[10px] font-mono text-ghost bg-surface-2 px-2 py-0.5 rounded border border-border-subtle">
                        Total: {group.items.length}
                      </div>
                      {expandedGroups.has(group.code) ? <ChevronDown className="w-4 h-4 text-ghost" /> : <ChevronRight className="w-4 h-4 text-ghost" />}
                    </div>
                  </button>
                      ...
                      <div className="bg-surface-2 border-b border-border-subtle flex items-center px-6 py-2">
                        <div className="w-10 whitespace-nowrap">
                          <input 
                            ...
                            checked={group.items.every(i => selectedCodes.has(i.code))}
                            onChange={() => toggleSelectAll(group.code)}
                          />
                        </div>
                        <div className="w-16 font-mono text-[10px] uppercase tracking-widest text-ghost ml-4 whitespace-nowrap">Item No.</div>
                        <div className="w-24 font-mono text-[10px] uppercase tracking-widest text-ghost whitespace-nowrap">Code</div>
                        <div className="flex-1 font-mono text-[10px] uppercase tracking-widest text-ghost whitespace-nowrap">Trade Item</div>
                      ...
                      {group.items.map((item, index) => (
                        ...
                        >
                          <div className="flex items-center gap-4 px-6 py-3.5">
                            <div className="w-10 whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                              <input 
                                type="checkbox"
                                className="rounded border-border-subtle bg-surface-base text-primary focus:ring-primary"
                            ...
                            <div className="flex-1 min-w-0 whitespace-nowrap">
                              <div className="text-[13px] font-medium text-main truncate">{cleanRichText(item.name)}</div>
                              <div className="flex items-center gap-3 mt-1">
                                <span className="text-[10px] font-mono text-ghost">per {cleanRichText(item.unit)}</span>
                                {item.outputQty && (
                            ...
                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                              {item.tier === 'global' && hasCapability('trade:view_global') && (
                                <button 
                                  onClick={(e) => {
                                    e.stopPropagation();
                              ...
                              {item.tier === 'company' && isGodMode && (
                                <button 
                                  onClick={async (e) => {
                                    e.stopPropagation();
                                    if (!confirm('Promote this trade item to Global Library?')) return;
                                    try {
                                      const { error } = await supabase
                                        .from('trade_items')
                                        .update({ library_tier: 'global', tenant_id: null })
                                        .eq('trade_code', item.code);
                                      if (error) throw error;
                                      alert('Promoted to Global successfully!');
                                      loadTradeLibrary();
                                    } catch (err: any) {
                              ...
                              {((item.tier === 'company' && hasCapability('trade:edit_company')) || isGodMode) && (
                                <>
                                  <button 
                                    onClick={(e) => {
                                      e.stopPropagation();
          ...
          <div className="fixed inset-y-0 right-0 w-[520px] bg-surface-1 border-l border-border-subtle shadow-2xl z-[70] flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-5 border-b border-border-subtle flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-[11px] text-ghost font-medium uppercase tracking-wider">{getGroupLabel(selectedItem.group)}</span>
                </div>
                <div className="text-lg font-bold leading-tight text-main">{selectedItem.name}</div>
              </div>
              <button 
                onClick={() => {
                  setSelectedItem(null);
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-surface-2 border border-border-subtle rounded-xl p-4 text-center">
                  <div className="font-mono text-[10px] uppercase tracking-widest text-ghost mb-1">Daily Output</div>
                  <div className="text-2xl font-bold text-main font-mono">{selectedItem.outputQty || '—'}</div>
                  <div className="text-[10px] text-ghost mt-1">{selectedItem.unit} / day</div>
                </div>
                <div className="bg-surface-2 border border-border-subtle rounded-xl p-4 text-center">
                  <div className="font-mono text-[10px] uppercase tracking-widest text-ghost mb-1">Labour Hours</div>
                  <div className="text-2xl font-bold text-main font-mono">{selectedItem.labHrs || '—'}</div>
                  <div className="text-[10px] text-ghost mt-1">hrs / {selectedItem.unit}</div>
                </div>
              </div>
              ...
              {selectedItem.teamSetup && (
                <div>
                  <div className="font-mono text-[10px] uppercase tracking-widest text-ghost mb-3 border-b border-border-subtle pb-2">Team Setup</div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3 border-b border-border-subtle pb-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-[10px] text-ghost">per {selectedItem.unit}</div>
                  </div>
                </div>
                  ...
                  {isEditingRecipe ? (
                    recipeDraft.length === 0 ? (
                      <div className="py-8 text-center text-ghost bg-surface-2 rounded-lg border border-dashed border-border-subtle">
                        <button onClick={handleAddRecipeRow} className="text-xs hover:text-main text-accent">+ Add your first resource</button>
                      </div>
                    ) : (
                      recipeDraft.map((res, idx) => (
                        <div key={idx} className="bg-surface-2 border border-border-subtle rounded-lg p-3 flex flex-col gap-3 group relative">
                          <button 
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="flex flex-col gap-1">
                              ...
                              <select 
                                ...
                                value={res.resource_type || 'material'}
                                onChange={(e) => handleUpdateRecipeDraft(idx, { resource_type: e.target.value })}
                              >
                                <option value="material">Material</option>
                                <option value="labour">Labour</option>
                                <option value="equipment">Equipment</option>
                            ...
                            <div className="flex flex-col gap-1">
                              <label className="text-[9px] uppercase font-bold text-ghost">Res Code</label>
                              <input 
                              ...
                              <div className="relative">
                                <select 
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const type = res.resource_type || 'material';
                                    let resource: any = null;
                                    
                                    switch(type) {
                                      case 'material': resource = libraryResources.materials.find(m => m.material_name === name); break;
                                      case 'labour': resource = libraryResources.labour.find(l => l.title === name); break;
                                      case 'equipment': resource = libraryResources.equipment.find(eq => eq.name === name); break;
                                      case 'vehicle': resource = libraryResources.vehicles.find(v => v.name === name); break;
                                      case 'fuel': resource = libraryResources.fuel.find(f => f.name === name); break;
                                    }

                                    if (resource) {
                                      handleUpdateRecipeDraft(idx, { 
                                        resource_name: name,
                                        resource_code: resource.material_code || resource.grade_code || resource.equipment_code || resource.vehicle_code || resource.fuel_code || '',
                                        resource_unit: resource.unit || (res.resource_type === 'labour' || res.resource_type === 'equipment' || res.resource_type === 'vehicle' ? 'day' : '')
                                      });
                                    } else {
                                      handleUpdateRecipeDraft(idx, { resource_name: name });
                                    }
                                  }}
                                >
                                  <option value="">-- Select {res.resource_type} --</option>
                                  {(res.resource_type === 'material' ? libraryResources.materials :
                                    res.resource_type === 'labour' ? libraryResources.labour :
                                    res.resource_type === 'equipment' ? libraryResources.equipment :
                                    res.resource_type === 'vehicle' ? libraryResources.vehicles :
                                    res.resource_type === 'fuel' ? libraryResources.fuel : []
                                  ).map((item: any) => (
                                    <option key={item.id} value={item.material_name || item.title || item.name}>
                                      {item.material_name || item.title || item.name}
                                    </option>
                                  ))}
                                </select>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            <div className="flex flex-col gap-1">
                              ...
                              <div className="flex items-center gap-1.5">
                                <input 
                                  ...
                                  value={res.consumption_rate || 0}
                                  onChange={(e) => handleUpdateRecipeDraft(idx, { consumption_rate: parseFloat(e.target.value) || 0 })}
                                />
                                <button 
                                  onClick={() => setShowCalculator({ index: idx, row: res })}
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <input 
                                ...
                                className="bg-surface-base border border-border-subtle text-xs px-2 py-1 rounded outline-none text-main"
                                value={res.waste_factor_pct || 0}
                                onChange={(e) => handleUpdateRecipeDraft(idx, { waste_factor_pct: parseFloat(e.target.value) || 0 })}
                              />
                            </div>
                          </div>
                        </div>
                    ...
                    <>
                      {isSyncing && enrichedResources.length === 0 ? (
                        <div className="py-8 text-center text-ghost">
                          ...
                          <div className="text-[10px]">Fetching library data...</div>
                        </div>
                      ) : enrichedResources.length === 0 ? (
                        <div className="py-8 text-center text-ghost bg-surface-2 rounded-lg border border-dashed border-border-subtle">
                          <Info className="w-6 h-6 opacity-20 mx-auto mb-2" />
                          <div className="text-xs">No resources defined for this recipe</div>
                        </div>
                      ) : (
                        (() => {
                          const groupsObj: Record<string, typeof enrichedResources> = {
                            'Material': [],
                            'Labour': [],
                            'Equipment': [],
                            'Vehicle': [],
                          ...
                          enrichedResources.forEach(res => {
                            const type = (res.resource_type || '').toLowerCase().trim();
                            if (type === 'material') {
                          ...
                          return Object.entries(groupsObj).map(([groupName, items]) => {
                            if (items.length === 0) return null;
                            return (
                              <div key={groupName} className="mb-4 last:mb-0">
                                <div className="flex items-center gap-1.5 mb-2 px-1">
                                  <span className="w-1.5 h-1.5 rounded-full bg-accent" />
                                  <span className="text-[10px] font-bold text-ghost uppercase tracking-wider">{groupName}</span>
                                  <span className="text-[9px] text-ghost/60 font-mono">({items.length})</span>
                                </div>
                                <div className="flex flex-col gap-2">
                                  {items.map((res, idx) => (
                                    <div key={idx} className="bg-surface-2 border border-border-subtle rounded-lg p-3 flex items-center gap-3 hover:border-border-accent/30 transition-colors">
                                      ...
                                      <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between">
                                          <div className="text-[13px] font-semibold text-main truncate">{res.resource_name}</div>
                                          {res.not_found && (
                                        ...
                                        <div className="flex items-center justify-between mt-0.5">
                                          <div className="flex items-center gap-2">
                                            <span className="text-[11px] font-mono text-dim">
                                              {res.consumption_rate} {res.resource_unit}
                                            </span>
                                            {res.waste_factor_pct && res.waste_factor_pct > 0 && (
              ...
              {selectedItem.notes && (
                <div>
                  <div className="font-mono text-[10px] uppercase tracking-widest text-ghost mb-3 border-b border-border-subtle pb-2">Field Notes</div>
            ...
            <div className="p-5 border-t border-border-subtle bg-surface-1 flex items-center justify-end gap-3">
              {isEditingRecipe ? (
                ...
                ((selectedItem.tier === 'company' && hasCapability('trade:edit_company')) || isGodMode) && (
                  <>
                    <button 
          ...
          <div className="bg-surface-1 border border-border-subtle rounded-2xl w-full max-w-5xl h-[85vh] overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-border-subtle flex items-center justify-between bg-surface-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-main">Import Analysis Overview</h3>
                  <p className="text-xs text-ghost">Review how items will be merged into the library</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-4 grid grid-cols-4 gap-4 bg-surface-base">
              <div className="bg-surface-2 p-3 rounded-xl border border-border-subtle">
                <p className="text-[10px] font-bold text-ghost uppercase tracking-wider mb-1">Total Items</p>
                <p className="text-xl font-bold text-main">{importPreviewRows.length}</p>
              </div>
              <div className="bg-primary/10 p-3 rounded-xl border border-primary/20">
                <p className="text-[10px] font-bold text-primary uppercase tracking-wider mb-1">New To Library</p>
              ...
              <div className="bg-warning/10 p-3 rounded-xl border border-warning/20">
                <p className="text-[10px] font-bold text-warning uppercase tracking-wider mb-1">Items To Update</p>
                <p className="text-xl font-bold text-warning">
                   {importPreviewRows.filter(r => r.status === 'update').length}
                </p>
              </div>
              <div className="bg-danger/10 p-3 rounded-xl border border-danger/20">
                <p className="text-[10px] font-bold text-danger uppercase tracking-wider mb-1">Items To Skip</p>
               ...
               <table className="w-full text-left border-collapse">
                 <thead>
                   <tr className="border-b border-border-subtle">
                     <th className="pb-3 text-[10px] font-bold text-ghost uppercase tracking-wider">Status</th>
                     <th className="pb-3 text-[10px] font-bold text-ghost uppercase tracking-wider">Group</th>
                     <th className="pb-3 text-[10px] font-bold text-ghost uppercase tracking-wider">Item Code</th>
                     <th className="pb-3 text-[10px] font-bold text-ghost uppercase tracking-wider">Item Name</th>
                     <th className="pb-3 text-[10px] font-bold text-ghost uppercase tracking-wider">Resource</th>
                   </tr>
                 </thead>
                 <tbody className="divide-y divide-surface-2">
                   {importPreviewRows.map((row, idx) => (
                     <tr key={idx} className="hover:bg-surface-2/50 transition-colors">
                       <td className="py-3">
                         {row.status === 'new' && <span className="px-2 py-0.5 rounded text-[9px] font-bold bg-primary/10 text-primary">NEW</span>}
                         {row.status === 'update' && <span className="px-2 py-0.5 rounded text-[9px] font-bold bg-warning/10 text-warning">UPDATE</span>}
                         {row.status === 'skip' && <span className="px-2 py-0.5 rounded text-[9px] font-bold bg-danger/10 text-danger">SKIP</span>}
                       </td>
                       <td className="py-3 text-xs text-dim">{row.trade_group}</td>
                       <td className="py-3 text-xs font-mono font-bold text-main">{row.trade_code}</td>
                       <td className="py-3 text-xs text-main">{row.trade_item}</td>
                       <td className="py-3 text-xs text-ghost">{row.resource_name}</td>
                     </tr>
                   ))}
                 </tbody>
               </table>
      ...
      {showCalculator && (
        <ConsumptionCalculator 
          ...
          onConfirm={(rate) => {
            handleUpdateRecipeDraft(showCalculator.index, { consumption_rate: rate });
