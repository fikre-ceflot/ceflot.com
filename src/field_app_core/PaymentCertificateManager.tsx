  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-main">Interim Payment Certificates</h2>
          <p className="text-sm text-dim">Manage subcontractor claims and payment approvals</p>
        </div>
        <button 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-2 border-b border-border-subtle">
                <th className="px-6 py-4 text-[10px] font-bold text-dim uppercase tracking-wider">Ref No.</th>
                <th className="px-6 py-4 text-[10px] font-bold text-dim uppercase tracking-wider">Subcontractor</th>
                <th className="px-6 py-4 text-[10px] font-bold text-dim uppercase tracking-wider">Period End</th>
                <th className="px-6 py-4 text-[10px] font-bold text-dim uppercase tracking-wider">Status</th>
                <th className="px-6 py-4 text-[10px] font-bold text-dim uppercase tracking-wider text-right">Net Amount</th>
                <th className="px-6 py-4 text-[10px] font-bold text-dim uppercase tracking-wider"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border-subtle">
              {loading ? (
                ...
                certificates.map((cert) => (
                  <tr key={cert.id} className="hover:bg-main/5 transition-colors group text-main">
                    <td className="px-6 py-4">
                      <div className="text-sm font-bold text-main">{cert.certificate_no}</div>
                      <div className="text-[10px] text-dim font-mono">{cert.id.split('-')[0]}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Building2 className="w-3.5 h-3.5 text-dim" />
                        <span className="text-sm text-main">{cert.subcontractors?.company_name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-ghost">{cert.period_end}</td>
                    <td className="px-6 py-4">
                      <span className={cn(
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="text-sm font-mono font-bold text-main">${cert.net_amount.toLocaleString()}</div>
                      <div className="text-[9px] text-dim">Gross: ${cert.gross_amount.toLocaleString()}</div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button 
  ...
  async function updateStatus(newStatus: string) {
    try {
      const { error } = await supabase
        ...
        .update({ 
          status: newStatus,
          certified_at: newStatus === 'certified' ? newStatus : null
        })
        .eq('id', cert.id);
      if (error) throw error;
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-dim">{cert.subcontractors?.company_name} • Period End: {cert.period_end}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-surface-3 border-b border-border-subtle">
                    <th className="px-4 py-3 text-[10px] font-bold text-dim uppercase tracking-wider">Activity</th>
                    <th className="px-4 py-3 text-[10px] font-bold text-dim uppercase tracking-wider text-right">Prev Qty</th>
                    <th className="px-4 py-3 text-[10px] font-bold text-dim uppercase tracking-wider text-right">To Date</th>
                    <th className="px-4 py-3 text-[10px] font-bold text-dim uppercase tracking-wider text-right">Current</th>
                    <th className="px-4 py-3 text-[10px] font-bold text-dim uppercase tracking-wider text-right">Rate</th>
                    <th className="px-4 py-3 text-[10px] font-bold text-dim uppercase tracking-wider text-right">Amount</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border-subtle">
                  {loading ? (
                    <tr><td colSpan={6} className="px-4 py-8 text-center text-dim animate-pulse">Loading items...</td></tr>
                  ) : items.map((item) => (
                    <tr key={item.id} className="hover:bg-main/5 transition-colors">
                      <td className="px-4 py-3">
                        <div className="text-xs font-medium text-main">{item.boq_items?.description}</div>
                        <div className="text-[9px] text-dim uppercase">{item.boq_items?.unit}</div>
                      </td>
                      <td className="px-4 py-3 text-xs text-dim text-right font-mono">{item.previous_qty.toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs text-main text-right font-mono">{item.total_qty_to_date.toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs text-primary text-right font-mono font-bold">{item.current_qty.toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs text-dim text-right font-mono">${item.rate.toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs text-main text-right font-mono font-bold">${item.amount.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-surface-1 border border-border-subtle rounded-xl p-6 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-mono font-bold text-main">${cert.gross_amount.toLocaleString()}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-dim">Retention (10%)</span>
                <span className="text-sm font-mono font-bold text-error">-${cert.retention_amount.toLocaleString()}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-dim">Deductions</span>
                <span className="text-sm font-mono font-bold text-error">-${cert.deductions_amount.toLocaleString()}</span>
              </div>
              <div className="h-px bg-border-subtle my-2" />
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold text-main">Net Payable</span>
                <span className="text-lg font-mono font-bold text-primary">${cert.net_amount.toLocaleString()}</span>
              </div>
            </div>
          </div>
